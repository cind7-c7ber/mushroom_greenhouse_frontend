...
function SkeletonCard() {
  return <div style={{ height: 76, borderRadius: 10, background: 'var(--c-bg-elevated)', animation: 'pulse 1.5s infinite' }} />
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: config.lineColor, marginBottom: 3 }}>
            {config.label}
          </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          {overallMeta && (
            <span style={{
              ...
              background: overallMeta.bg, border: `1px solid ${overallMeta.border}`, color: overallMeta.color,
            }}>
              {overallMeta.label}
            </span>
          )}
          <span style={{
            ...
            padding: '3px 8px', borderRadius: 5,
            background: config.tagBg, border: `1px solid ${config.tagBorder}`, color: config.tagColor,
          }}>
            {section}
          </span>
        </div>
      ...
      <div style={{ padding: 14 }}>
        {loading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10 }}>
            {METRICS.map(m => <SkeletonCard key={m} />)}
          </div>
        ) : data ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }} className="sm:grid-cols-3 xl:grid-cols-5">
            {METRICS.map(m => (
          ...
          <div style={{ padding: '24px 0', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 11, color: 'var(--c-tx-muted)', marginTop: 4, opacity: 0.6 }}>Waiting for MQTT payload</p>
          </div>
        )}
      </div>
      ...
      {/* Footer */}
      {data?.timestamp && (
        <div style={{ padding: '8px 18px 12px', borderTop: '1px solid var(--c-bg-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span className="label-caps" style={{ fontSize: 9 }}>Last reading</span>
          <span style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace', color: 'var(--c-tx-muted)' }}>
            {new Date(data.timestamp).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', second: '2-digit' })}
          </span>
        </div>
      )}
    </div>
